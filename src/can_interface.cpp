...
CanInterface::CanInterface(const std::string& interface_name, int bitrate, CanFrameCallback callback)
...
bool CanInterface::initialize()
{
  ...
  {
    std::cerr << "Error setting interface " << interface_name_ << " baudrate to " << bitrate_ << std::endl;
    close(socket_fd_);
    socket_fd_ = -1;
    return false;
  }
  ...
  {
    std::cerr << "Error binding socket to interface " << interface_name_ << ": " << strerror(errno) << std::endl;
    close(socket_fd_);
    socket_fd_ = -1;
    return false;
  }
...
{
  std::string cmd = "sudo ip link set " + interface_name_ + " type can bitrate " + std::to_string(baudrate);
  int ret = system(cmd.c_str());
  return (ret == 0);
}
...
bool CanInterface::is_running() const
{
  return running_;
}
